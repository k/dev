...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    function openTrove(uint _maxFeePercentage, uint _AMPLAmount, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        ...
        vars.ICR = LiquityMath._computeCR(_AMPLAmount, vars.compositeDebt, vars.price);
        vars.NICR = LiquityMath._computeNominalCR(_AMPLAmount, vars.compositeDebt);

        if (isRecoveryMode) {
            _requireICRisAboveCCR(vars.ICR);
        } else {
            _requireICRisAboveMCR(vars.ICR);
            uint newTCR = _getNewTCRFromTroveChange(_AMPLAmount, true, vars.compositeDebt, true, vars.price);  // bools: coll increase, debt increase
            _requireNewTCRisAboveCCR(newTCR); 
        }

        // Set the trove struct's properties
        contractsCache.troveManager.setTroveStatus(msg.sender, 1);
        contractsCache.troveManager.increaseTroveColl(msg.sender, _AMPLAmount);
        ...
        _activePoolAddColl(contractsCache.activePool, _AMPLAmount);
        ...
        emit TroveUpdated(msg.sender, vars.compositeDebt, _AMPLAmount, vars.stake, BorrowerOperation.openTrove);
        emit LUSDBorrowingFeePaid(msg.sender, vars.LUSDFee);
    }

    // Send AMPL as collateral to a trove
    function addColl(uint _AMPLAmount, address _upperHint, address _lowerHint) external payable override {
        _adjustTrove(msg.sender, 0, _AMPLAmount, 0, false, _upperHint, _lowerHint, 0);
    }

    // Send AMPL as collateral to a trove. Called by only the Stability Pool.
    function moveETHGainToTrove(address _borrower, uint _amount, address _upperHint, address _lowerHint) external payable override {
        _requireCallerIsStabilityPool();
        _adjustTrove(_borrower, 0, _amount, 0, false, _upperHint, _lowerHint, 0);
    }
    ...
    function adjustTrove(uint _maxFeePercentage, uint _collWithdrawal, uint _collDeposit, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external payable override {
    ...
    */
    ...
    function _activePoolAddColl(IActivePool _activePool, uint _amount) internal {
        bool success = amplToken.transferFrom(msg.sender, address(_activePool), _amount);
        require(success, "BorrowerOps: Sending AMPL to ActivePool failed");
    }
    ...
    function _requireSingularCollChange(uint _collWithdrawal, uint _collDeposit) internal pure {
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _collDeposit, uint _LUSDChange) internal pure {
